...
def eval_vln():
    ...
    for i in range(4):
        ...
        return_action, generate_time, return_llm_output = evaluator.step(0,
                                        ...
                                        instruction,
                                        run_model=run_model)
        if isinstance(return_llm_output, str) and len(return_llm_output.strip()) > 0:
    ...
    print(f"final llm_output len={len(llm_output)}")
    if len(action_seq) == 0:
        print("!!!!!!!!!!!!!!!!!task finish!!!!!!!!!!!!!!!!!!!!!")
        return jsonify({'action': [0]})
    
    action_payload = action_seq.tolist() if hasattr(action_seq, 'tolist') else list(action_seq)
    return jsonify({'action': action_payload})
    
if __name__ == '__main__':
    global local_rank
    parser = argparse.ArgumentParser()
    parser.add_argument("--model_path", type=str, default="/home/zhuoqi/StreamVLN/checkpoints/StreamVLN_Video_qwen_1_5_r2r_rxr_envdrop_scalevln_real_world")
